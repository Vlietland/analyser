...
describe('DEFAULT_VIEW_STATE', () => {
  it('should have correct default values', () => {
    expect(DEFAULT_VIEW_STATE.zoom).toBe(1.0);
...
describe('updateViewState', () => {
  ...
  it('should handle partial updates correctly', () => {
    ...
    expect(newState.zoom).toBe(DEFAULT_VIEW_STATE.zoom);
  ...
  it('should overwrite existing properties with updates', () => {
    const initialState: ViewState = { zoom: 2, rotationX: 1, rotationZ: 1, zFactor: 3 };
    const updates: Partial<ViewState> = { zoom: 0.5, rotationZ: 0 };
